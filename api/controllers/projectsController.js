...
// create project and push into a user's projects
// you need to send the user data (id) back in the req.body
// you can adjust this code if you wish
...
function projectsUpdate(req, res){
  ...
  Project.findByIdAndUpdate({ _id: id }, req.body, function(err, project){
    if (err) return res.status(500).send(err);
    if (!project) return res.status(404).send(err);
